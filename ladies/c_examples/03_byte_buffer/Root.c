...
FOREIGN_DECL(Value, Root_Bx_raw_new_buffer, int64_t, cap, {
...
FOREIGN_DECL(Value, Root_Bx_raw_put_u8, Value, buf, int64_t, byte, {
    buffer_put_u8(buf, (uint8_t)byte);
    return buf;
})
// raw_move : Raw_Buffer -> Raw_Slice
FOREIGN_DECL(Value, Root_Bx_raw_move, Value, buf, { return buffer_move(buf); })
// raw_slice_len : Raw_Slice -> Int
FOREIGN_DECL(int64_t, Root_Bx_raw_slice_len, Value, s, { return (int64_t)slice_len(s); })
// raw_get_u8 : Raw_Slice -> Int -> Int
FOREIGN_DECL(int64_t, Root_Bx_raw_get_u8, Value, s, int64_t, i, {
    return (int64_t)slice_get_u8(s, (size_t)i);
})
// raw_get_i32_le : Raw_Slice -> Int -> Int
FOREIGN_DECL(int64_t, Root_Bx_raw_get_i32_le, Value, s, int64_t, off, {
