#!/usr/bin/env sh
# Run the language-feature panel and CHECK each program's output against its
# `expected` file. Same convention as ladies/examples/run.sh: each `start` prints
# a "##TC" sentinel before its results; the compiler epilogue prints "####"/"$$$$".
# We carve the program output out from between and compare it to <name>/expected.
#
